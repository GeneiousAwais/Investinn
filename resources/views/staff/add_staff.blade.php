...
<form class="row g-3 needs-validation" id="projectsDetailsForm" novalidate method="POST" action="{{ route('users.update',isset($user_info) ? $user_info->id : '') }}">
    ...
    <form class="row g-3 needs-validation" id="projectsDetailsForm" novalidate method="POST" action="{{ route('users.store') }}">
        ...
        <div class="col-md-4 col-sm-12 {{ request()->query('tab') == 'personaldetails_edit' ? 'd-none' : '' }}">
            <div class="form-label-group in-border">
                <input type="password" class="form-control @if($errors->has('password')) is-invalid @endif" id="investorPassword" name="password" placeholder="Please enter " value="{{ isset($user_info) ? $user_info->password : old('password') }}" required>
                <label for="investorPassword" class="form-label fs-5 fs-lg-1">Password <span class="text-danger">*</span></label>
                <div class="invalid-tooltip">
        ...
        <div class="col-md-4 col-sm-12 {{ request()->query('tab') == 'personaldetails_edit' ? 'd-none' : '' }}">
            <div class="form-label-group in-border">
                <input type="password" class="form-control @if($errors->has('conf_password')) is-invalid @endif" id="investorConfirmPassword" name="conf_password" placeholder="Please enter " value="{{ isset($user_info) ? $user_info->password : old('conf_password') }}" required>
                <label for="investorConfirmPassword" class="form-label fs-5 fs-lg-1">Confirm Password <span class="text-danger">*</span></label>
                <div class="invalid-tooltip">
        ...
        <div class="col-md-1 col-sm-12">
            <a href="{{ isset($user_info) ? trim($string) : '/files/user_profiles/user-dummy-img.jpg' }}" target="_blank" ><img class="rounded avatar-xs header-profile-user mt-1" src="{{ isset($user_info) ? trim($path) : '/files/user_profiles/user-dummy-img.jpg' }}" alt="Header Avatar"></a>
        </div>
        ...
        <div class="col-12 text-end">
            <input type="hidden" class="form-control" id="form_info" name="form_info"  value="staff">
            <input type="hidden" class="form-control" id="userTypeId" name="user_type_id"  value="4">
            <input id="base64data" type="hidden" name="base64data" value="">
            <button class="btn btn-primary" type="submit">Save Record</button>
            <a href="{{ route('staff.index') }}" class="btn btn-light bg-gradient waves-effect waves-light">Cancel</a>
        </div>
    </form>
@push('header_scripts')
@endpush
